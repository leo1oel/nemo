...
#
# Firstmate is a git repo of itself with linked worktrees: crewmate worktrees and
# secondmate homes are all herdr worktrees of the same repo ($FM_ROOT), while the
# PRIMARY checkout (the repo root firstmate operates from) is a normal checkout on
# a real branch - normally the default branch, main. The "worktree tangle" failure
# mode is a crewmate spawned to work on firstmate ITSELF branching and committing in
# the primary checkout instead of its own disposable worktree, stranding the primary
# on a feature branch (e.g. fm/readme-restructure-d3).
#
