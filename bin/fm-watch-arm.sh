...
# surface, then prints one reason line and exits; benign wakes are absorbed in
# bash without ending the run. While state/.afk exists the daemon owns triage and
# the watcher exits on every wake for the daemon to classify. Reliability depends
# on arming through a mechanism that SURVIVES the call and NOTIFIES on exit, so
# firstmate must run this script as the harness's own tracked background task
# (e.g. run_in_background). Run it as its own standalone background task, never
# bundled onto the tail of another command. NEVER fire it and forget with a shell
# `&` inside another call: that backgrounded child is reaped when the call
# returns, leaving NO watcher running and a false "already running" off the dying
# process. That exact mistake silently took supervision down for ~30 minutes.
#
