...
# edits. This discovers live secondmate homes from state/*.meta (kind=secondmate),
# backfills home= from data/secondmates.md for older meta records, and reuses the
# same propagate_inheritable_config machinery a secondmate spawn runs, but
# deliberately does not fast-forward tracked files and does not nudge running
# secondmates. Warnings-only skips exit 0; real propagation errors exit non-zero.
set -u
...
seen_homes=""
errors=0
while IFS='|' read -r id home _handle meta; do
  [ -n "$id" ] || continue
  if [ -z "$home" ]; then
