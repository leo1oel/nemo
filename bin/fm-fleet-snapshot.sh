...
. "$SCRIPT_DIR/fm-herdr-lib.sh"  # fm_herdr_pane_exists for the endpoint-liveness read
...
# Fork-native status parsing. Upstream ships these in fm-classify-lib.sh; this
# fork's classify-lib does not, and the fork's crewmates do not emit [key=...]
# decision markers, so open decisions are tracked simply as the last unresolved
# needs-decision/blocked line, cleared by a later resolving verb.
...
status_open_decisions() {  # <status-file> -> TSV(key,verb,note) of the open decision, if any
  local f=$1 line verb last_verb='' last_note=''
  [ -f "$f" ] || return 0
  while IFS= read -r line || [ -n "$line" ]; do
    case "${line//[[:space:]]/}" in '') continue ;; esac
    verb=$(status_line_verb "$line")
    case "$verb" in
      needs-decision|blocked) last_verb=$verb; last_note=$(status_line_note "$line") ;;
      done|failed|working|checks-passed|passed|checks|running|fixing|ci) last_verb=''; last_note='' ;;
    esac
  done < "$f"
  [ -n "$last_verb" ] && printf '%s\t%s\t%s\n' "$last_note" "$last_verb" "$last_note"
}
...
task_json_lines() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    if [ -n "$target" ]; then
      if fm_herdr_pane_exists "$target" >/dev/null 2>&1; then
    ...
    if [ "$kind" = secondmate ] && [ -n "$target" ]; then
      if fm_herdr_pane_exists "$target" >/dev/null 2>&1; then agent_alive=alive; else agent_alive=dead; fi
    fi
