...
MODE=$("$FM_ROOT/bin/fm-project-mode.sh" "$REPO")
...
**Verify isolation before anything else.** Run \`pwd -P\` and \`git rev-parse --show-toplevel\`; both must resolve to the disposable herdr worktree you were launched in, not the primary checkout firstmate operates from.
