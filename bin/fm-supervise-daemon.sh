...
#          FM_SUPERVISOR_TARGET     captain's herdr pane id (override; otherwise
#                                   auto-discovered from HERDR_PANE_ID, inherited
#                                   when /afk launches the daemon from the captain)
...
#   2. $HERDR_PANE_ID — herdr sets this in every agent pane's environment; the
#      daemon inherits the captain's pane id when /afk launches it from
#      firstmate's own pane.
# Returns the resolved target on stdout; returns 1 if neither is set (herdr has
# no named-pane fallback, so the caller errors out — a missing target is a typo).
...
# --- herdr pane layer (replaces tmux capture-pane / send-keys / cursor) ------
# Resolve a wake-reason "window" to a herdr pane id (handle). A crewmate window
# is "fm-<id>"; its handle is recorded as handle= in state/<id>.meta. Anything
# else is assumed to already be a pane id (the supervisor target = the captain's
# HERDR_PANE_ID). Empty if an fm-<id> has no live meta.
_handle_for() {  # <window-or-pane> <state>
  local w=$1 state=$2 id
  case "$w" in
    fm-*) id="${w#fm-}"; sed -n 's/^handle=//p' "$state/$id.meta" 2>/dev/null | head -1 ;;
    *)    printf '%s' "$w" ;;
  esac
}
...
# Find a live crewmate whose task id matches the given stale-marker key by
# enumerating state/*.meta (the herdr equivalent of tmux's window list). Returns
# the wake-reason window name "fm-<id>" so pane_is_busy can resolve its handle.
window_for_task() {  # <task-key>
  local key=$1 meta id
  for meta in "$(_state_root)"/*.meta; do
    [ -e "$meta" ] || continue
    id=$(basename "$meta" .meta)
    [ "$(_stale_key "$id")" = "$key" ] && { printf 'fm-%s' "$id"; return 0; }
  done
  return 1
}
...
inject_msg() {  # <message> [state]
  ...
  if ! herdr pane send-text "$h" "$msg" >/dev/null 2>&1; then
    log "inject failed: herdr pane send-text returned non-zero"
    return 1
  fi
  sleep "$sleep_s"
  i=0
...
fm_super_main() {
  ...
  # Priority: FM_SUPERVISOR_TARGET override > $HERDR_PANE_ID (inherited from the
  # captain's pane that launched the daemon). No named-pane fallback under herdr.
  # Exporting the result into FM_SUPERVISOR_TARGET makes inject_msg (which reads
  # that env var) use the discovered pane without an extra global.
  ...
  if discovered=$(discover_supervisor_target); then
    ...
    echo "warn: could not auto-discover supervisor pane (set FM_SUPERVISOR_TARGET, or launch the daemon from the captain's pane so it inherits HERDR_PANE_ID)" >&2
  fi
  ...
  if ! pane_exists "$TARGET"; then
    echo "error: supervisor target '$TARGET' does not resolve to a herdr pane; set FM_SUPERVISOR_TARGET (the captain's HERDR_PANE_ID)" >&2
