...
#       Provision <home> as an isolated firstmate home. If <home> is "-", provision
#       a fresh herdr worktree of the firstmate repo ($FM_ROOT) as the home. herdr
#       never recycles a worktree, so the home survives with no live process until
#       it is explicitly removed (retirement or seed rollback) via
#       "herdr worktree remove". Projects are cloned
...
#       A herdr-provisioned home also records its herdr workspace id in
#       <home>/.fm-secondmate-home.workspace so retirement can remove the worktree;
#       a plain <home> path supplied explicitly stays a plain directory home with no
#       herdr workspace.
#       Seeding is transactional: on validation, clone, init, or registry failure,
#       generated briefs, new homes, new project clones, and registry edits are
#       rolled back. herdr-provisioned homes are removed only when the rollback
#       target is safe; a failed removal warns because the worktree may still exist.
...
# Sidecar next to the marker: records the herdr workspace id for a herdr-provisioned
# home, so retirement removes the worktree (and closes its workspace). Absent for a
# plain directory home, which teardown must never herdr-remove.
SUB_HOME_WS_MARKER=".fm-secondmate-home.workspace"

# Shared herdr json helper (mirrors bin/fm-backend.sh _jget): extract a dotted path
# from JSON on stdin; missing -> empty, never an error.
_jget() {
  python3 -c '
import sys, json
...
validate_seed_leaf_files() {
  ...
  abs_home=$(resolved_path "$home")
  for label in "data/projects.md" "data/charter.md" "$SUB_HOME_MARKER" "$SUB_HOME_WS_MARKER"; do
    path="$home/$label"
    if [ -L "$path" ]; then
...
acquire_herdr_home() {
  local id=$1 j path ws
  # Provision a fresh herdr worktree of the firstmate repo as the home. herdr never
  # recycles a worktree, so the home survives with no live process until it is
  # explicitly removed (retirement or seed rollback). The worktree path and its
  # herdr workspace id come straight from the create json. Prints "<path>\t<workspace>".
  j=$(cd "$FM_ROOT" && herdr worktree create --cwd "$FM_ROOT" --branch "secondmate-$id" --label "sm-$id" --no-focus --json) || {
    echo "error: 'herdr worktree create' failed to provision a firstmate home" >&2
    return 1
  }
  path=$(printf '%s' "$j" | _jget result.worktree.path)
  ws=$(printf '%s' "$j" | _jget result.worktree.open_workspace_id)
  if [ -z "$path" ] || [ -z "$ws" ]; then
    echo "error: could not parse 'herdr worktree create' output (path/workspace missing)" >&2
    printf '%s\n' "$j" >&2
    return 1
  fi
  printf '%s\t%s\n' "$path" "$ws"
...
ensure_home() {
  local id=$1 requested=$2 home acquired
  if [ "$requested" = "-" ]; then
    acquired=$(acquire_herdr_home "$id") || return 1
    home=${acquired%%	*}
...
clone_project() {
  ...
  mode=$(FM_HOME="$FM_HOME" FM_DATA_OVERRIDE="$DATA" "$FM_ROOT/bin/fm-project-mode.sh" "$project")
  if [ -e "$dst" ]; then
...
seed_remove_herdr_home() {
  local home=$1 ws=$2 abs_home
  abs_home=$(seed_rollback_target "$home" "herdr-provisioned home") || return 0
  if ! command -v herdr >/dev/null 2>&1; then
    echo "warning: failed to remove herdr-provisioned home $abs_home during seed rollback; herdr command not found" >&2
    return 0
  fi
  if [ -z "$ws" ]; then
    echo "warning: no herdr workspace recorded for $abs_home during seed rollback; worktree may still exist" >&2
    return 0
  fi
  # Removes the worktree AND closes its workspace+pane in one call.
  herdr worktree remove --workspace "$ws" --force >/dev/null 2>&1 || {
    echo "warning: failed to remove herdr-provisioned home $abs_home during seed rollback; worktree may still exist" >&2
    return 0
  }
}
...
seed_rollback() {
  ...
  if [ -n "${SEED_HOME:-}" ] && [ "$SEED_HOME" != "/" ]; then
    if [ "$SEED_HOME_ACQUIRED" = 1 ]; then
      seed_remove_herdr_home "$SEED_HOME" "$SEED_HOME_WORKSPACE"
...
project_mode_in_home() {
  ...
  mode=$(FM_ROOT_OVERRIDE='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' FM_PROJECTS_OVERRIDE='' FM_CONFIG_OVERRIDE='' FM_HOME="$home" "$FM_ROOT/bin/fm-project-mode.sh" "$project")
  printf '%s\n' "$mode"
}
...
seed_home() {
  local id=$1 requested_home=$2 requested_abs home acquired projects_csv project project_dst charter_summary charter_scope
  shift 2
  ...
  # A herdr-provisioned home records its workspace id beside the marker so teardown
  # can remove the worktree; a plain directory home leaves this absent (and any stale
  # copy from a prior plain-home seed is cleared).
  if [ "$SEED_HOME_ACQUIRED" = 1 ] && [ -n "$SEED_HOME_WORKSPACE" ]; then
    printf '%s\n' "$SEED_HOME_WORKSPACE" > "$home/$SUB_HOME_WS_MARKER"
  else
    rm -f "$home/$SUB_HOME_WS_MARKER" 2>/dev/null || true
  fi
