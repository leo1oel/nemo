...
# (e.g. primary config/crew-dispatch.json makes a secondmate use the same
# dispatch profile rules).
#
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json}"
...
# Guard: an inherited item may only land where git would ignore it. A home is a
# worktree of this same repo, so a non-ignored destination would dirty the home
# and could leak local config into a commit. A destination outside any work tree
# has no such hazard and is allowed.
