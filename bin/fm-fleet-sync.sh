...
# FM_ROOT locates this script's own bin/ (honoring FM_ROOT_OVERRIDE for tests);
# operational dirs come from FM_HOME, so the project clones we sync are the home's
# projects/, not always the primary checkout's. This matches fm-project-mode.sh
# and fm-guard.sh, so a secondmate home syncs its own clones rather than the main's.
FM_ROOT="${FM_ROOT_OVERRIDE:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
FM_HOME="${FM_HOME:-${FM_ROOT_OVERRIDE:-$FM_ROOT}}"
"$FM_ROOT/bin/fm-guard.sh" || true

# Shared git-lock staleness proof (fm_lock_is_provably_stale). A ref rewrite
# killed mid-write can strand a .git/packed-refs.lock that fails every later
# fetch; the guard below recovers from it. FM_LOCK_LOG_PREFIX tags the lib's
# stderr as fleet-sync output.
# shellcheck source=bin/fm-lock-lib.sh
. "$FM_ROOT/bin/fm-lock-lib.sh"
# Read by the sourced lib's fm_lock_log; shellcheck cannot see that use here.
...
# renaming it - a timed-out sync kill, or a teardown's process kills - leaves a
# lock that makes the next sync's fetch fail with Git's
...
project_label() {
  case "$PROJ" in
    "$FM_HOME"/projects/*) basename "$PROJ" ;;
...
fetch_with_packed_refs_lock_guard() {
  ...
  if [ -n "$lock" ] && [ -e "$lock" ]; then
    if fm_lock_is_provably_stale "$lock" "$PROJ" "$FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS"; then
      ...
      FETCH_OUTPUT=$(git -C "$PROJ" fetch origin --prune --quiet 2>&1); rc=$?
      [ "$rc" -eq 0 ] && echo "$label: fetch succeeded after stale packed-refs lock cleanup" >&2
      return "$rc"
    fi
