#!/usr/bin/env bash
# Claude PreToolUse transport for the cd-guard command policy.
#
...
# Claude PreToolUse payload, invokes that policy, and renders the deny response.
# It never executes, sources, evaluates, or expands the command.
...
# here; none of those harnesses run in this fork).
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/fm-cd-pretool-check.sh [--claude]
#   bin/fm-cd-pretool-check.sh --command '<cmd>'   # CLI mode (tests)
#
...
# marker - a $ immediately followed by a single quote (ANSI-C $'...') or a double
# quote (bash locale $"...") - delegates too, because the classifier decodes
# those and can reconstruct cd from bytes this substring test cannot see. This
# marker set is COUPLED to the classifier's decoder set in
...
# home but remains inert when the secondmate home is itself a linked worktree.
