...
//
// Fork note (Claude-only): upstream also protects a Codex-only
// bin/fm-watch-checkpoint.sh. This fork has no checkpoint concept - Claude arms
// the watcher as a tracked background task and is re-invoked on its exit - so
// the "checkpoint" protected identity is dropped here; only "arm" (valid
// standalone) and "watch" (forbidden direct) remain.
//
...
// Fork note (Claude-only): upstream also blesses `source config/x-mode.env` (and
// its `[ -f ... ]` guard) as a setup node before the arm, an X-mode startup form.
// This fork has no X-mode and no source-a-config-before-arming pattern, so that
// blessing is dropped - only bare `cd <dir>` and `export VAR=val` setup nodes
// remain (tighter than upstream). The context param is retained for call parity.
