...
Default fields: schema, home, generated, prs, in_flight{id,kind,state,doing},
  secondmates{id,state,doing,reason}, decisions_open{id,key,verb,summary,owner},
  landed{id,what,artifact,owner}, gates{id,title,blocked_by,reason,owner},
...
landed merges this home's Done with registered secondmate homes' Done, bounded by
  ...
  order while preserving each home's internal newest-first order; sparse homes do not
  waste capacity. --all-landed reveals the full global newest-first set.
For every registered secondmate, the validated structured state from its own home is
  authoritative: its in-flight child work and open decisions surface as the
  secondmate's, and a home that could not be read is shown as state unknown.
Opt-in surfaces: --fields bodies|paths|actions|endpoints, --all-in-flight,
  --all-decisions, --all-secondmates, --all-landed, --all-reports, --all-queued,
  --all-recorded-prs, --all-unhealthy, --all-pr-repos, --include-prs (adds candidate_prs).
...
# One timestamp for the whole projection: passing it into the snapshot keeps the
# recursive secondmate summaries' `generated` stamp consistent with this run, which
# the snapshot then verifies. --all-landed / --all-secondmates lift the snapshot's
# own bounds so nothing is dropped before bearings applies its tighter view caps.
...
MODEL=$(printf '%s' "$SNAP" | jq \
  ...
  | ([ .tasks[] as $t | select($t.kind != "secondmate") | ($t.hints.open_decisions // [])[]
     ...
     + [ (.secondmate_current.records // [])[] as $m | $m.decisions_open[]?
         | {id:($m.id + "/" + .id),key,verb,summary:(.summary | trunc(90)),owner:$m.id} ]) as $decisions_all
  | ([ .backlog.records[]
  ...
  | . + {omitted: (
      [ (if $f_bodies then empty else {surface:"backlog item bodies", reveal:"--fields bodies"} end),
        ...
        (([ ($snap.secondmate_current.records // [])[] | select(.current.state == "unknown") ] | length) as $n | if $n > 0 then {surface:("secondmate home state unavailable for \($n) home(s)"), reveal:"inspect the listed secondmate homes"} else empty end),
