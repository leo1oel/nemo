...
#
# The hazard: a no-mistakes gate agent runs inside a firstmate checkout with a
# free shell, so it auto-loads firstmate's AGENTS.md, adopts the captain identity,
# and - seeing crew "in flight" - reaches for fm-spawn.sh/fm-send.sh/fm-teardown.sh
# to "delegate" and "reconcile" the shared worktree. It has real capability because
...
#
# A NORMAL firstmate session - a real primary checkout, a real herdr crew
# worktree - has NEITHER signal and is COMPLETELY unaffected: the function returns
# 0 and the lifecycle proceeds exactly as before.
#
...
# would run that suite FROM a gate worktree (cwd git-common-dir under
# .no-mistakes/repos/*.git, and possibly NO_MISTAKES_GATE set) - the exact
# environment this guard refuses. So both signals could fire during firstmate's
# own validation and break unrelated tests. FM_GATE_REFUSE_BYPASS=1 makes the
# guard a no-op; this fork has no shared tests/lib.sh, so every test that drives
# these three scripts against its temp-sandbox fleet exports the bypass itself.
...
# adopting firstmate's identity - it never runs firstmate's tests, so it never
# carries the bypass; and the adversarial case (an agent that would deliberately
# set it) is covered by no-mistakes' neutral-execution-context and the
# HEAD-continuity guard. The dedicated tests/fm-gate-refuse.test.sh strips the
# bypass so it still verifies real refusal.
#
