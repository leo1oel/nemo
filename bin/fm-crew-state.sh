...
# no-mistakes run-step attributed to this crew's branch, else the herdr pane
...
#
# Logic, in order:
#   1. Resolve worktree + pane handle + kind from state/<id>.meta.
...
#      herdr pane busy-signature (fm-herdr-lib.sh) + the status log's last line.
#   5. Missing meta or torn-down worktree: report unknown · none. If no run is
#      attributed to this crew, a dead/closed pane also reports unknown · none
#      rather than trusting a stale status log.
#
...
# shell - so a finished crew whose pane has closed still reports its run-step
# state (e.g. done) instead of being masked as unknown.
pane_readable() {  # <handle>
...
# is no run to consult, so a dead/unreadable pane means the crew is gone: report
# unknown rather than trusting a possibly-stale status log as the current state.
[ -n "$HANDLE" ] || emit unknown none "no pane recorded"
pane_readable "$HANDLE" || emit unknown none "pane gone: $HANDLE"

# Secondmates idle on their own watcher (idle pane = healthy), so the busy
# signature is not meaningful for them; read their state from the status log only.
if [ "$KIND" != secondmate ] && fm_herdr_pane_is_busy "$HANDLE"; then
