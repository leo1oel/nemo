...
# firstmate repo and every registered secondmate home on the herdr backend.
#
...
#     origin; a herdr secondmate home (a worktree of this repo checked out on
#     its lease branch `secondmate-<id>`) fast-forwards that branch the same way.
...
#     nudge-secondmates lists exactly the live secondmates that advanced, by
#     their `fm-<id>` send target (fm-send.sh resolves the herdr pane handle).
...
# Add a secondmate home as a herdr-style worktree of the firstmate repo, checked
# out on its lease branch `secondmate-<id>` (matching fm-home-seed.sh), plus its
# state meta. The meta carries a herdr pane handle, not a tmux window. Args:
# world id.
add_sm() {
  local w=$1 id=$2
  git -C "$w/main" worktree add -q -b "secondmate-$id" "$w/$id" main
  {
    printf 'handle=pane-%s\n' "$id"
...
test_updates_main_and_secondmate() {
  ...
  assert_contains "$out" "nudge-secondmates: fm-sm1" "updated secondmate is nudged by its send target"
  ...
  # Firstmate stays on its default branch; secondmate stays on its lease branch.
  [ "$(git -C "$w/main" symbolic-ref --short HEAD 2>/dev/null)" = "main" ] \
    || fail "firstmate left its default branch"
  [ "$(git -C "$w/sm1" symbolic-ref --short HEAD 2>/dev/null)" = "secondmate-sm1" ] \
    || fail "secondmate worktree left its lease branch"
...
test_diverged_secondmate_skipped() {
  ...
  w=$(new_world t5)
  add_sm "$w" sm1
  # Local commit on the secondmate's lease branch makes it diverge from origin.
  printf 'fork work\n' > "$w/sm1/AGENTS.md"
  git -C "$w/sm1" add -A
...
test_registry_backstop() {
  ...
  git -C "$w/main" worktree add -q -b "secondmate-reg1" "$w/reg1" main
  ...
  pass "T7 secondmate resolved from registry without a live nudge target"
}
...
# --- T12: herdr-specific - a home on an unexpected branch is skipped --------
# The lease-branch acceptance must not degrade into "any branch": a secondmate
# home checked out on some branch that is neither the default nor its own lease
# branch is skipped, not force-advanced.
test_secondmate_unexpected_branch_skipped() {
  local w out before
  w=$(new_world t12)
  add_sm "$w" sm1
  # Move the home off its lease branch onto an unrelated branch.
  git -C "$w/sm1" checkout -q -b stray
  before=$(git -C "$w/sm1" rev-parse HEAD)
  bump_origin "$w" instr

  out=$(run_update "$w")

  assert_contains "$out" "secondmate sm1: skipped: on stray, expected secondmate-sm1" "off-lease-branch home skipped"
  assert_not_contains "$out" "fm-sm1" "off-lease-branch home is not nudged"
  [ "$(git -C "$w/sm1" rev-parse HEAD)" = "$before" ] \
    || fail "off-lease-branch home HEAD moved"
  pass "T12 secondmate home on an unexpected branch is skipped, not forced"
}
