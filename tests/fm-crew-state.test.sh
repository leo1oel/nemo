...
# reads the AUTHORITATIVE source (a matching no-mistakes run-step, else the herdr
...
trap cleanup EXIT
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/fm-crew-state-tests.XXXXXX")

# A stable git identity for the throwaway repos, independent of the caller's config.
...
# Write a state/<id>.meta from key=value line arguments.
fm_write_meta() {  # <meta-path> <key=value>...
  local path=$1; shift
  : > "$path"
  local kv
...
# fake `herdr` (serves a busy or idle pane via agent_status). The fake no-mistakes
# mirrors the real command surface the helper uses: `axi status`, `axi status
# --run <id>`, and a bare `axi` (the run list). Each returns the matching
# FM_FAKE_AXI_* env text. The fake herdr mirrors `pane get` (existence +
# agent_status) and `pane read` (the footer fallback render).
...
case "${1:-} ${2:-}" in
  "pane get")
    [ "${FM_FAKE_PANE_MISSING:-0}" = 1 ] && { echo '{"error":{"code":"pane_not_found"}}'; exit 1; }
    if [ "${FM_FAKE_BUSY:-0}" = 1 ]; then st=working; else st=idle; fi
...
test_active_run_is_authoritative() {
  ...
  fm_write_meta "$d/state/feat-a.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_stale_needs_decision_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-b.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_stale_blocked_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-bb.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_genuine_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-c.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_scalar_gate_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-cs.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_gate_block_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-cb.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_ci_ready_done_log_beats_monitoring_run() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-ci.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_terminal_passed() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-d.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_terminal_failed() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-e.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_no_run_busy_pane() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-h.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_no_run_idle_pane_uses_log() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-i.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
test_dead_pane_ignores_stale_status_log() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-dead.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
  printf 'done: old completion event\n' > "$d/state/feat-dead.status"
  FM_FAKE_AXI_STATUS=""
  FM_FAKE_AXI_LIST=""
  FM_FAKE_PANE_MISSING=1
...
test_dead_pane_still_reports_terminal_run_step() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-dead-done.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
  ...
  FM_FAKE_PANE_MISSING=1   # the crew's pane has closed
...
test_dead_pane_still_reports_active_run_step() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-dead-act.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
...
SH
  ...
  fm_write_meta "$d/state/feat-timeout.meta" "handle=p1" "worktree=$d/wt" "kind=ship"
  FM_FAKE_BUSY=1
  start=$SECONDS
...
test_scout_skips_run_lookup() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/scout-j.meta" "handle=p1" "worktree=$d/wt" "kind=scout"
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/gone-k.meta" "handle=p1" "worktree=$d/no-such-worktree" "kind=ship"
  local out rc
