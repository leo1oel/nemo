...
1. **Run the updater:**
   ...
   bin/fm-update.sh
   ```
   It fast-forwards this firstmate repo's default branch from origin, then fast-forwards every registered secondmate home the same way.
   On the herdr backend each secondmate home is a herdr worktree of this same repo, checked out on its own lease branch `secondmate-<id>`; the fast-forward advances only that worktree's lease branch and never touches another worktree's checkout or the shared default branch.
   ...
   - `nudge-secondmates: <send-targets...>|none`
...
3. **Nudge each updated live secondmate.**
   ...
   ```sh
   bin/fm-send.sh <send-target> 'firstmate was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.'
   ```
   Each `<send-target>` is the secondmate's `fm-<id>` task target; `fm-send.sh` resolves it to the live herdr pane.
...
- **Fast-forward only.**
  A target that has diverged, is dirty, is offline, or is on a non-default / non-lease branch is skipped and reported, never forced or stashed.
