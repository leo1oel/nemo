...
A stall episode wakes the first mate at most once: staleness hashes the pane tail excluding the ticking TUI footer (the same last-6-non-blank-line window the busy check reads), and a surfaced stale or fired wedge escalation is not repeated until the pane changes or goes busy - ending the episode - and then stalls again.
