...
| `fm-fleet-snapshot.sh`   | Read-only structured (`--json`, schema `fm-fleet-snapshot.v1`) snapshot of the whole fleet: backlog, one row per task meta with reconciled current state and herdr endpoint presence, open-decision hints, scout reports, a secondmate-landed roll-up, and `secondmate_current` (each registered secondmate's authoritative CURRENT state, derived by recursively running the `--secondmate-home-summary` mode against its own home, bounded by `FM_SNAPSHOT_SECONDMATES`/`FM_SNAPSHOT_SECONDMATE_TIMEOUT`); no locks, wakes, or mutations |
| `fm-fleet-view.sh`       | Human renderer over `fm-fleet-snapshot.sh --json` (a fleet table); `--json` passes the underlying snapshot through |
| `fm-bearings-snapshot.sh` | Compact TOON projection of the canonical snapshot for a "pick up where I left off" read; LOCAL-ONLY by default (zero network), with `--include-prs` as the sole opt-in that touches GitHub; surfaces each registered secondmate's authoritative home state (`secondmates`, plus owner-tagged in-flight/decisions/gates/landed, `--all-secondmates` to expand); feeds the `bearings` skill (`/bearings`) |
...
| `fm-crew-state.sh`       | Print one stable current-state line for a crew by reconciling its matching no-mistakes run-step (a running ci step is checked against its own `axi logs` so a green merge-deferred PR reads done, not "validating"), even when the pane has closed, with pane and status-log fallback |
