...
FM_ARM_ATTACH_POLL=0.5      # seconds between health polls while fm-watch-arm rides an already-healthy watcher
...
FM_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=3        # fetch retries on an orphaned packed-refs.lock before the provably-stale removal
FM_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=1  # wait between those retries (a transient lock self-clears)
FM_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=30      # min mtime age before a held-open-by-nobody packed-refs.lock is treated as stale
